...
body +=`</div>`
    const channelInfoText = {
      ...
      byTeam: 'Including a team (MLB only, by abbreviation, in a comma-separated list if more than 1) will include all of its broadcasts, or if that team is not broadcasting the game, it will include the national broadcast or opponent broadcast if available. By default it will also include affiliate (MiLB) games for those organizations; add &includeLevels=mlb to limit results to the MLB level only (no minor league affiliates), see "By team, MLB only" below. Channels/games subject to blackout will be omitted by default.',
      byTeamMLBOnly: 'Same as By team, but adds &includeLevels=mlb to limit results to MLB-level games for the specified team(s) only (no AAA/AA/A+/A affiliate games included). Combine with &audio_track=radio for radio audio.',
...
body += `
    ...
    body += '<div class="channelRow"><div>' + channelInfo('By team, MLB only', 'byTeamMLBOnly') + '</div><div class="channelLinks"><a class="channelLink" href="' + http_root + '/channels.m3u?mediaType=' + mediaType + '&resolution=' + resolution + '&includeTeams=' + include_teams + '&includeLevels=mlb' + content_protect_b + '">m3u</a><a class="channelLink" href="' + http_root + '/guide.xml?mediaType=' + mediaType + '&includeTeams=' + include_teams + '&includeLevels=mlb' + content_protect_b + '">xml</a><a class="channelLink" href="' + http_root + '/calendar.ics?mediaType=' + mediaType + '&includeTeams=' + include_teams + '&includeLevels=mlb' + content_protect_b + '">ics</a></div></div>' + "\n"
