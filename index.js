...
app.get('/', async function(req, res) {
  try {
    ...
    body += '<p><span class="tooltip">By team<span class="tooltiptext">Including a team (MLB only, by abbreviation, in a comma-separated list if more than 1) will include all of its broadcasts, or if that team is not broadcasting the game, it will include the national broadcast or opponent\'s broadcast if available. By default it will also include affiliate (MiLB) games for those organizations; add &includeLevels=mlb to limit results to the MLB level only (no minor league affiliates) — see "By team, MLB only" below. Channels/games subject to blackout will be omitted by default. See below for an additional option to override that.</span></span>: <a href="' + http_root + '/channels.m3u?mediaType=' + mediaType + '&resolution=' + resolution + '&includeTeams=' + include_teams + content_protect_b + '">channels.m3u</a> and <a href="' + http_root + '/guide.xml?mediaType=' + mediaType + '&includeTeams=' + include_teams + content_protect_b + '">guide.xml</a> and <a href="' + http_root + '/calendar.ics?mediaType=' + mediaType + '&includeTeams=' + include_teams + content_protect_b + '">calendar.ics</a></p>' + "\n"

    body += '<p><span class="tooltip">By team, MLB only<span class="tooltiptext">Same as above, but adds &includeLevels=mlb to limit results to MLB-level games for the specified team(s) only (no AAA/AA/A+/A affiliate games included). Combine with &audio_track=radio for radio audio.</span></span>: <a href="' + http_root + '/channels.m3u?mediaType=' + mediaType + '&resolution=' + resolution + '&includeTeams=' + include_teams + '&includeLevels=mlb' + content_protect_b + '">channels.m3u</a> and <a href="' + http_root + '/guide.xml?mediaType=' + mediaType + '&includeTeams=' + include_teams + '&includeLevels=mlb' + content_protect_b + '">guide.xml</a> and <a href="' + http_root + '/calendar.ics?mediaType=' + mediaType + '&includeTeams=' + include_teams + '&includeLevels=mlb' + content_protect_b + '">calendar.ics</a></p>' + "\n"
